...
>[!info] C++ 代码 面对大量数据 读取 输出操作，最好用scanf 和 printf，耗时会小很多：
>```cpp
>scanf("%d%d", &a, &b);
>printf("%d\n", sum);
>```
...
### Container **find** element \<algorithm\>
...
if (it != v.end()) 
{
	cout << "Found " << target << " at index " << distance(v.begin(), it) << '\n'; 
} 
else 
{
	std::cout << target << " not found in vector.\n"; 
}
```